...
- `apps/web` — Next.js 16 (App Router, TypeScript, Tailwind v4, shadcn/ui,
  ...
  loan products, EMI/eligibility calculators) with SEO and analytics, plus
  a signed-in customer portal (login/register/OTP/password reset,
  dashboard, profile, session/device management) — a Server-Actions-based
  BFF that moves the browser onto httpOnly session cookies in front of
  the token API below.
